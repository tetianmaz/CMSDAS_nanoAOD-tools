...
class ExampleAnalysis(Module):
    ...
    def beginJob(self, histFile=None, histDirName=None):
        ...
        self.h_denom = ROOT.TH1F('denom', 'denom', 50, 0, 200)
        self.h_num = ROOT.TH1F('num', 'num', 50, 0, 200)
    ...
    def analyze(self, event):
        ...
        if len(electron) > 0:
            idx = 0
            for lep in electron:
                if (lep.genPartFlav == 15 and lep.mvaFall17V2Iso_WP80 and abs(lep.eta < 2.5)): #electron selection
                    Tauele_exists=True
                    if lep.pt > ele_pt:
                        ele_idx = idx
                        ele_pt = lep.pt
        ...
        if len(tau) > 0:
            for lep in tau:
                if (abs(lep.eta < 2.3) and lep.pt > 20. and lep.genPartFlav == 5 and abs(lep.dz) < 0.2 and lep.idDeepTau2017v2p1VSe >= 8 and lep.decayMode != 5 and lep.decayMode != 6 and lep.decayMode != 7): #add more requirements here 
                    Tauh_exists=True
...
preselection = "Electron_pt.size()>0 && Tau_pt.size()>0"
...
p = PostProcessor(".", files, cut=preselection, branchsel=None, modules=[
                  ExampleAnalysis()], noOut=False, histFileName="histeffcross.root", histDirName="plots")
p.run()
